...
class EnergyBalanceModel:
    ...
    def __init__(
        ...
        if len(self.ocean_heat_transfer) != self.n_temperature_boxes:
            raise ValueError("ocean_heat_capacity and ocean_heat_transfer must be arrays of the same shape.")
        if self.n_temperature_boxes < 2:
            raise ValueError("At least two ocean layers must be specified in the energy balance model.")
