...
def forcing_from_input(data, time, column=None, **kwargs):
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if column is not None:
            ...
            if data.shape[1] != 1:
                raise ExternalForcingError(
                    "If not providing a column name to the forcing DataFrame, then only one column other than the time index should be provided."
                )
            base_data = data.iloc[:, 0].values.squeeze()
    elif isinstance(data, np.ndarray):
        if data.shape[1] != 2:
            raise ExternalForcingError(
                "If specifying a numpy array of external forcing, a (n_timestep, 2) shape array is required where the first column is time and the second column is forcing. The shape of the array you provided is {}.".format(
                    data.shape
                )
            )
        base_time = data[:, 0]
        ...
        raise ExternalForcingError(
            "Please provide a filepath (to a csv), a DataFrame or a numpy array"
        )
