...
# hack to skip RMSE step
valid_temp=np.arange(1600000, dtype=int)
...
toa_in = np.load(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/prior_runs/"
    "toa_imbalance_2014-2023_mean.npy"
)
fari_in = np.load(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/prior_runs/"
...
samples["TOA"] = scipy.stats.norm.rvs(
    loc=1.16717054, scale=0.33852825, size=10**5, random_state=28016539
)
...
target_toa = scipy.stats.gaussian_kde(samples["TOA"])
prior_toa = scipy.stats.gaussian_kde(toa_in)
post1_toa = scipy.stats.gaussian_kde(toa_in[valid_temp])
post2_toa = scipy.stats.gaussian_kde(draws[0]["TOA"])
...
if plots:
    ...
    ax[2, 2].plot(
        np.linspace(start, stop, 1000),
        prior_toa(np.linspace(start, stop, 1000)),
        color=colors["prior"],
        label="Prior",
        lw=2,
    )
    ax[2, 2].plot(
        np.linspace(start, stop, 1000),
        post1_toa(np.linspace(start, stop, 1000)),
        color=colors["post1"],
        label="Temperature RMSE",
        lw=2,
    )
    ax[2, 2].plot(
        np.linspace(start, stop, 1000),
        post2_toa(np.linspace(start, stop, 1000)),
        color=colors["post2"],
        label="All constraints",
        lw=2,
    )
    ax[2, 2].plot(
        np.linspace(start, stop, 1000),
        target_toa(np.linspace(start, stop, 1000)),
        color=colors["target"],
        label="Target",
        lw=2,
    )
    ax[2, 2].set_xlim(start, stop)
    ax[2, 2].set_ylim(0, 2.0)
    ax[2, 2].set_title("TOA energy imbalance")
    ax[2, 2].set_yticklabels([])
    ax[2, 2].set_xlabel("W m$^{-2}$, 2014-2023 mean")
...
print("TOA energy imbalance:", np.percentile(draws[0]["TOA"], (5, 50, 95)))
