...
future = scmdata.ScmRun(rcmip_emissions_file, lowercase_cols=True).filter(scenario=scenarios, variable=rcmip_variables, region='World').interpolate(times_future).timeseries(time_axis="year")
future = future/1000
...
new_index = pd.MultiIndex.from_tuples(list(zip(*list(map(list,zip(*arrays))))), names=future.index.names)
...
scenarios_harmonised.to_csv('../../../../../data/emissions/co2_ffi_afolu_harmonized.csv', index=False)
...
for scenario in scenarios:
    ...
    1750:2020].values.squeeze()
    data_fut = scenarios_harmonised.loc[
        (scenarios_harmonised["scenario"]==scenario) &
        (scenarios_harmonised["variable"]=="Emissions|CO2|Energy and Industrial Processes"),
    2021:2499].values.squeeze()
