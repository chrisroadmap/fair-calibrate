...
"""AR6 calibration of FaIR 2.1, concentration driven"""

# Let's not waste time running beyond 2021. We can re-run scenarios after the RMSE step.
import os
...
if __name__ == "__main__":
    ...
    volcanic_forcing[271] = volcanic_forcing[270]

    solar_forcing = df_solar['erf'].loc[1750:2021].values
    ...
    temp_out = np.ones((172, samples)) * np.nan
    ...
    for ibatch, batch_start in enumerate(range(0, samples, batch_size)):
        ...
        config[ibatch]['scaling_Volcanic'] = df_scaling.loc[batch_start:batch_end-1, 'Volcanic'].values.squeeze()
        config[ibatch]['scaling_solar_trend'] = df_scaling.loc[batch_start:batch_end-1, 'solar_trend'].values.squeeze()
        config[ibatch]['scaling_solar_amplitude'] = df_scaling.loc[batch_start:batch_end-1, 'solar_amplitude'].values.squeeze()
        ...
        config[ibatch]['kappa1'] = df_cr.loc[batch_start:batch_end-1, 'kappa1'].values
        config[ibatch]['kappa2'] = df_cr.loc[batch_start:batch_end-1, 'kappa2'].values
        config[ibatch]['kappa3'] = df_cr.loc[batch_start:batch_end-1, 'kappa3'].values
        config[ibatch]['epsilon'] = df_cr.loc[batch_start:batch_end-1, 'epsilon'].values
        config[ibatch]['gamma'] = df_cr.loc[batch_start:batch_end-1, 'gamma'].values
        config[ibatch]['sigma_eta'] = df_cr.loc[batch_start:batch_end-1, 'sigma_eta'].values
        config[ibatch]['sigma_xi'] = df_cr.loc[batch_start:batch_end-1, 'sigma_xi'].values
        config[ibatch]['seed'] = np.arange(seedgen+batch_start*seedstep, seedgen+batch_end*seedstep, seedstep, dtype=int)
        config[ibatch]['forcing_4co2'] = df_cr.loc[batch_start:batch_end-1,'F_4xCO2']
        config[ibatch]['iirf_0'] = df_cc.loc[batch_start:batch_end-1, 'r0'].values.squeeze()
        config[ibatch]['iirf_airborne'] = df_cc.loc[batch_start:batch_end-1, 'rA'].values.squeeze()
        config[ibatch]['iirf_uptake'] = df_cc.loc[batch_start:batch_end-1, 'rU'].values.squeeze()
        config[ibatch]['iirf_temperature'] = df_cc.loc[batch_start:batch_end-1, 'rT'].values.squeeze()
        config[ibatch]['beta'] = df_aci.loc[batch_start:batch_end-1, 'beta'].values.squeeze()
        config[ibatch]['shape_so2'] = df_aci.loc[batch_start:batch_end-1, 'shape_so2'].values.squeeze()
        config[ibatch]['shape_bc'] = df_aci.loc[batch_start:batch_end-1, 'shape_bc'].values.squeeze()
        config[ibatch]['shape_oc'] = df_aci.loc[batch_start:batch_end-1, 'shape_oc'].values.squeeze()
        config[ibatch]['scaling_CO2'] = df_scaling.loc[batch_start:batch_end-1, 'CO2'].values.squeeze()
        config[ibatch]['scaling_CH4'] = df_scaling.loc[batch_start:batch_end-1, 'CH4'].values.squeeze()
        config[ibatch]['scaling_N2O'] = df_scaling.loc[batch_start:batch_end-1, 'N2O'].values.squeeze()
        config[ibatch]['scaling_minorGHG'] = df_scaling.loc[batch_start:batch_end-1, 'minorGHG'].values.squeeze()
        config[ibatch]['scaling_stwv'] = df_scaling.loc[batch_start:batch_end-1, 'Stratospheric water vapour'].values.squeeze()
        config[ibatch]['scaling_contrails'] = df_scaling.loc[batch_start:batch_end-1, 'Contrails'].values.squeeze()
        config[ibatch]['scaling_lapsi'] = df_scaling.loc[batch_start:batch_end-1, 'Light absorbing particles on snow and ice'].values.squeeze()
        config[ibatch]['scaling_landuse'] = df_scaling.loc[batch_start:batch_end-1, 'Land use'].values.squeeze()
        ...
        config[ibatch]['ari_EESC'] = df_ari.loc[batch_start:batch_end-1, 'Equivalent effective stratospheric chlorine']
        ...
        config[ibatch]['ozone_EESC'] = df_ozone.loc[batch_start:batch_end-1, 'Equivalent effective stratospheric chlorine']
    ...
    np.save(f'../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/prior_runs/temperature_1850-2021.npy', temp_out, allow_pickle=True)
    ...
    np.save(f'../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/prior_runs/cumulative_emissions_co2_2020_minus_1750.npy', co2_out, allow_pickle=True)
