...
def run_fair(cfg):
    ...
    da = da_concentration.loc[dict(config="unspecified", scenario=scenarios)][:352, ...]
...
#    initialise(f.concentration, f.species_configs["baseline_concentration"])
    ...
    return (
        (
            np.average(
                f.temperature[271:292, :, :, 0],
                weights=average_20yr,
                axis=0,
            ) - np.average(
                f.temperature[245:266, :, :, 0],
                weights=average_20yr,
                axis=0,
            )
        ),
