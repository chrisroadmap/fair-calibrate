...
update = pd.DataFrame(columns=range(1750, 2024))

# CO2 from Global Carbon Project
co2_df = pd.read_csv(
    "../../../../../data/emissions/gcp_v2023_co2_1750-2023.csv", index_col=0
)
...
primap_df = pd.read_csv(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/emissions/"
    "primap-histtp-2.5.1_1750-2022.csv",
    index_col=0,
)
...
inv_df = pd.read_csv(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/emissions/"
    "minor_ghg_inverse_1750-2023.csv"
)
...
# co2
update.loc["Emissions|CO2|Energy and Industrial Processes", 1750:2023] = co2_df.loc[
    "1750":"2023", "fossil emissions excluding carbonation"
].values
update.loc["Emissions|CO2|AFOLU", 1750:2023] = co2_df.loc[
    "1750":"2023", "land-use change emissions"
].values
...
for specie in species:
    update.loc[f"Emissions|{specie}", 1750:2023] = slcf_df[
        names[specie]
    ].values.squeeze()
...
for specie in species_minor:
    update.loc[f"Emissions|{specie}", 1750:2023] = inv_df[specie].values.squeeze()
...
for specie in species_split:
    update.loc[f"Emissions|{specie}", 1750:2023] = split_df.loc[
