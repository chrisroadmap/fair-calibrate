...
for im, model in enumerate(models):
    ...
    params["$c_1$"][im], params["$c_2$"][im] = df.loc[
        condition, "C1":"C2"
    ].values.squeeze()
    (
        params[r"$\kappa_1$"][im],
        params[r"$\kappa_2$"][im],
    ) = df.loc[condition, "kappa1":"kappa2"].values.squeeze()
...
if plots:
    ...
    pl.savefig(
        ...
        "ebm2_distributions.png"
    )
    pl.savefig(
        f"../../../../../plots/fair-{fair_v}/v{cal_v}/{constraint_set}/"
        "ebm2_distributions.pdf"
    )
    pl.close()
...
ebm_sample = kde.resample(size=int(samples * 1.5), seed=2181882)
...
for isample in tqdm(range(len(ebm_sample.T)), disable=1 - progress):
    ebm = EnergyBalanceModel(
        ocean_heat_capacity=ebm_sample[1:3, isample],
        ocean_heat_transfer=ebm_sample[3:5, isample],
        deep_ocean_efficacy=ebm_sample[5, isample],
        gamma_autocorrelation=ebm_sample[0, isample],
        sigma_xi=ebm_sample[7, isample],
        sigma_eta=ebm_sample[6, isample],
        forcing_4co2=ebm_sample[8, isample],
        stochastic_run=True,
    )
    ...
    h_mat[:3, :3] = -eb_matrix
    h_mat[:3, 3:] = q_mat
    h_mat[3:, 3:] = eb_matrix.T
    g_mat = scipy.sparse.linalg.expm(h_mat)
    q_mat_d = g_mat[3:, 3:].T @ g_mat[:3, 3:]
    ...
    try:
        scipy.stats.multivariate_normal.rvs(size=1, mean=np.zeros(3), cov=q_mat_d)
    except:  # noqa: E722
