...
# 
# note here we calibrate to the ECLIPSE emissions supplied by Camilla. For concentration
# of non-CH4 species we use SSP2-4.5. For observations we use IGCC 2023
...
df_rcmip_emis = pd.read_csv(rcmip_emissions_file)
df_emis = pd.read_csv(eclipse_file)
...
# Camilla has fixed the NOx units issue
print(df_emis)
emis_species = ["CO", "VOC", "NOx"]
for species in emis_species:
    input[species] = (
        df_emis.loc[
            (df_emis["scenario"] == "MESSAGE-GLOBIOM___SSP2-45")
...
emis_ch4 = (
    df_emis.loc[
        (df_emis["scenario"] == "MESSAGE-GLOBIOM___SSP2-45-CLE-CH4")
...
for ssp in [
    ...
    emis_ch4_ssps[ssp] = (
        df_rcmip_emis.loc[
            (df_rcmip_emis["Scenario"] == ssp)
            & (df_rcmip_emis["Variable"].str.endswith("CH4"))
            & (df_rcmip_emis["Region"] == "World"),
            "1750":"2100",
        ]
        .interpolate(axis=1)
        .values.squeeze()
...
if plots:
    ...
    ax[1].set_title("(b) CH$_4$ concentration SSP2-4.5")
