...
co2_df = pd.read_csv(f'../../../../../data/emissions/gcp_v2023_co2_1750-2022.csv', index_col=0)
...
primap_df = pd.read_csv(f'../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/emissions/primap-histtp-2.5_1750-2022.csv', index_col=0)

# Split of HFCs and PFCs
split_df = pd.read_csv(f'../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/emissions/hfcs_pfcs_split_1750-2022.csv', index_col=0)

# GFED
gfed41s_df = pd.read_csv(f'../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/emissions/gfed4.1s_1997-2022.csv', index_col=0)

# pre-1997 biomass burning
bb_df = pd.read_csv('../../../../../data/emissions/global-biomass-burning-emissions.csv', index_col=0)

# inverse Montreal
inv_df = pd.read_csv(f'../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/emissions/minor_ghg_inverse_1750-2022.csv')
...
update.loc['Emissions|CH4', 1750:1996] = (
    primap_df.loc['CH4', '1750':'1996'].values +
    bb_df.loc[1750:1996, "CH4"].values
)
update.loc['Emissions|CH4', 1997:2022] = (
    primap_df.loc['CH4', '1997':'2022'].values +
    gfed41s_df.loc[1997:2022, "CH4"].values
)

# n2o
update.loc['Emissions|N2O', 1750:1996] = (
    primap_df.loc['N2O', '1750':'1996'].values +
    bb_df.loc[1750:1996, "N2O"].values
)
update.loc['Emissions|N2O', 1997:2022] = (
    primap_df.loc['N2O', '1997':'2022'].values +
    gfed41s_df.loc[1997:2022, "N2O"].values
)
...
for specie in species_split:
    update.loc[f"Emissions|{specie}", 1750:2022] = split_df.loc[split_df.index.str.endswith(specie.replace('-', '')), :].values.squeeze()


units = ["Gt CO2/yr", "Gt CO2/yr", "Mt CH4/yr", "Mt N2O/yr", "kt SF6/yr", "kt NF3/yr", "Mt SO2/yr",
    "Mt CO/yr", "Mt VOC/yr", "Mt NO2/yr", "Mt BC/yr", "Mt OC/yr", "Mt NH3/yr"]
units = units + [f'kt {specie.replace("-","")}/yr' for specie in species_minor + species_split]
