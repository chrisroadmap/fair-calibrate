...
df_emissions = pd.read_csv(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/emissions/all_scaled_1750-2024.csv"
)

for specie in df_emissions.variable:
    find_me = specie.split("|")[1]
    if find_me in df_species_configs.index and find_me != "CO2":
        df_species_configs.loc[find_me, 'baseline_emissions'] = df_emissions.loc[df_emissions["variable"] == specie, "1750"].values[0]
