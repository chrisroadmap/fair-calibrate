...
for i in tqdm(range(samples), disable=1 - progress):
    rmse_temp[i] = rmse(
        gmst[:170],
        temp_in[1:171, i] - np.average(temp_in[:52, i], weights=weights, axis=0),
    )
...
if plots:
    ...
    ax.plot(np.arange(1850.5, 2021), gmst, color="k", label="Best estimate historical")
    ax.set_xlim(1850, 2020)
    ax.set_ylim(-1, 4)
    ax.set_ylabel("°C relative to 1850-1900")
    ax.axhline(0, color="k", ls=":", lw=0.5)
