...
samples["OHC"] = scipy.stats.norm.rvs(
    loc=434.9, scale=110.4 / NINETY_TO_ONESIGMA, size=10**5, random_state=43178
)
...
accepted = pd.DataFrame(
    {
        ...
        "temperature 1995-2014": np.average(
            temp_in[145:166, valid_temp_af], weights=weights_20yr, axis=0
        )
...
post1_temp = scipy.stats.gaussian_kde(
    np.average(temp_in[145:166, valid_temp_af], weights=weights_20yr, axis=0)
    - np.average(temp_in[:52, valid_temp_af], weights=weights_51yr, axis=0)
)
...
# warming baselines
df_warming = pd.DataFrame(data=draws[0]["temperature 1995-2014"], index=draws[0].index, columns = ["temperature 1995-2014"]).sort_index()
df_warming.to_csv(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{constraint_set}/posteriors/"
