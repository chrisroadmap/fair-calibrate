...
# doing some experiments with flat10, ssp585 and 1pctco2, we find that some combinations
# coming from the sampling result in some low airborne fractions in high emissions
# scenarios, because either the rU + rA or rT term are negative and lead to declining
# airborne fraction with increasing emissions and warming. rT can be negative (but not too much so)
# since CanESM5 has a negative coefficient for this parameter, and this is also the case 
# in the original alpha/beta/gamma decomposition so is not an artefact of the fair 
# sampling method. rA + rU should not be, so we can and should remove these combinations
...
cc_sample = kde.resample(size=int(samples*1.1), seed=2421911)

# chuck out rA + rU < 0
cc_sample[:, cc_sample[1, :] + cc_sample[3, :] < 0] = np.nan
