...
for specie in species:
    ...
    input_obs["temp"] = gmst[:275]  # 1750-2020 timepoints

    emis_obs = df_emis_obs.loc[
        df_emis_obs["variable"] == f"{renames[specie]}", "1750":"2024"
    ].values.squeeze()  # 1750-2024 timepoints
    ...
    def find_scale_factor(sf):
        conc_estimated = np.zeros(275)
        gas_boxes = 0
        airborne_emissions = 0

        for i in range(275):
            conc_estimated[i], gas_boxes, airborne_emissions = one_box(
                emis_obs[i] * sf[0],
                gas_boxes,
                airborne_emissions,
                burden_per_emission,
        ...
        return conc_estimated[-1] - input_obs[specie][-1]
