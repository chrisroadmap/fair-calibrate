...
if plots:
    for model in models:
        pl.plot(np.arange(1750, 2025), conc_ch4[model][:275], label=model)
...
input_obs = {}
input_obs["CH4"] = df_conc_obs["CH4"].values[:275]
input_obs["N2O"] = df_conc_obs["N2O"].values[:275]
input_obs["VOC"] = df_emis_obs.loc[(df_emis_obs["variable"]=="VOC") & (df_emis_obs["scenario"]=='ssp370'), '1750':'2024'].values.squeeze()
input_obs["NOx"] = df_emis_obs.loc[(df_emis_obs["variable"]=="NOx") & (df_emis_obs["scenario"]=='ssp370'), '1750':'2024'].values.squeeze()
input_obs["temp"] = gmst[:275]

emis_ch4_obs = df_emis_obs.loc[
    (df_emis_obs["variable"] == "CH4") & (df_emis_obs["scenario"] == 'ssp370'), "1750":"2024"
].values.squeeze()
...
def fit_precursors(x, rch4, rnox, rvoc, rhc, rn2o, rtemp, rbase):
    conc_ch4 = np.zeros(275)  # 1751-2025 timebounds
    gas_boxes = 0  # should use correct pi value for CMIP6
    airborne_emissions = 0
...
p, cov = scipy.optimize.curve_fit(
    fit_precursors, invect, input_obs["CH4"][:275], bounds=(low - gap, high + gap)
)
...
conc_ch4["best_fit"] = np.zeros(275)
gas_boxes = 0
airborne_emissions = 0
...
if plots:
    ...
    ax[1].plot(
        np.arange(1750, 2025), conc_ch4["best_fit"], color="0.5", label="Best fit", lw=1
    )
    ax[1].plot(
        np.arange(1750, 2025), input_obs["CH4"], color="k", label="observations", lw=1
    )
    ax[1].set_ylabel("ppb")
    ax[1].set_xlim(1750, 2025)
