...
# TODO: update with 2024 from Blair
# we also did this in v1.4.5 so no worries
df_temp = pd.read_csv("../../../../../data/forcing/ssp_strawman_warming.csv")
gmst = df_temp["ssp245"].values
...
for specie in species:
    input_obs = {}
    input_obs[specie] = df_conc_obs[specie].values[:275]  # 1750-2024 timepoints
    input_obs["temp"] = gmst[:275]  # 1750-2024 timepoints
...
#        print(df_emis)
        ...
        for i in range(351):
            conc_ssp[ssp][i], gas_boxes, airborne_emissions = one_box(
                emis_ssp[i] * scalings_df.loc["historical_best", renames[specie]],
                gas_boxes,
                airborne_emissions,
                burden_per_emission,
                lifetime,
                ...
                timestep=1,
                natural_emissions_adjustment=natural_emissions_adjustment * scalings_df.loc["historical_best", renames[specie]],
            )
    emis_ssp = (
        df_emis.loc[
            (df_emis["variable"] == f"{renames[specie]}") & (df_emis["scenario"]==ssp), "1750":"2100"
    ...
    conc_unscaled_hist = np.zeros(275)
    natural_emissions_adjustment = emis_ssp[0]
    gas_boxes = 0
    airborne_emissions = 0
    for i in range(275):
