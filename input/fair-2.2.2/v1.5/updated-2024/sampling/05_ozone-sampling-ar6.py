...
for ispec, species in enumerate(emitted_species):
    species_out[species] = df_emis.loc[df_emis["variable"] == f"{species}", '1750':'2024'].values.squeeze()

for ispec, species in enumerate(concentration_species):
    species_out[species] = df_conc[species].values[:275]

species_df = pd.DataFrame(species_out, index=range(1750, 2025))
...
if plots:
    ...
    pl.plot(np.arange(1750.5, 2025), forcing, label="best estimate fit", color="0.5")
