...
if __name__ == "__main__":
    ...
    # still using CMIP6 solar because there is no updated solar dataset before 1850
    df_solar = pd.read_csv(
        "../../../../../data/forcing/solar_erf_timebounds.csv", index_col="year"
    )
    # the volcanic is from IGCC 2024 and includes Hunga Tonga
    df_volcanic = pd.read_csv(
        "../../../../../data/forcing/volcanic_ERF_annual_timebounds_1750-2025.csv",
        index_col="year",
    )

    volcanic_forcing = df_volcanic["volcanic_ERF"].loc[1750:2025].values
    solar_forcing = df_solar["erf"].loc[1750:2025].values
    ...
    # a bit of space
    temp_out = np.ones((176, samples)) * np.nan
