...
def run_fair(cfg):
    ...
    # solar and volcanic forcing - could be done in fill_from_csv but we'll use the files we have
    fill(
        f.forcing,
    ...
    # since we have a hodge-podge of different data sources, we have to do this the old way
    # climate response
    fill(
    ...
    # species level
    # start by filling the defaults with the calibrated values
    # then override parameters for the ensemble.
    ...
    return (
        ...
        np.average(
            f.forcing[255:266, 0, :, 23],
            weights=np.array([0.5, 1, 1, 1, 1, 1, 1, 1, 1, 1, 0.5]),
            axis=0,
        ),
        np.average(
            f.forcing[255:266, 0, :, 24],
            weights=np.array([0.5, 1, 1, 1, 1, 1, 1, 1, 1, 1, 0.5]),
            axis=0,
        ),
        f.ebms.ecs,
