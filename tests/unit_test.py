...
def test_ebm_init_array_mismatch_error():
    with pytest.raises(ValueError):
        ebm = EnergyBalanceModel(
            ocean_heat_capacity = [2, 10, 75],
            ocean_heat_transfer = [1.0, 3.0]
        )
...
def test_ebm_emergent_parameters():
    ...
    # )
    timescales = np.loadtxt(os.path.join(HERE, "test_data", "ebm3_cams-csm1-0_timescales.txt"))
    response_coefficients = np.loadtxt(os.path.join(HERE, "test_data", "ebm3_cams-csm1-0_response_coefficients.txt"))
    ecs_tcr = np.loadtxt(os.path.join(HERE, "test_data", "ebm3_cams-csm1-0_ecs_tcr.txt"))
    np.testing.assert_allclose(EBM_CAMS_STOCHASTIC.timescales, timescales)
    np.testing.assert_allclose(EBM_CAMS_STOCHASTIC.response_coefficients, response_coefficients)
    np.testing.assert_allclose(np.array([EBM_CAMS_STOCHASTIC.ecs, EBM_CAMS_STOCHASTIC.tcr]), ecs_tcr)
...
def test_ebm_run():
    ...
    # )
    temperature = np.loadtxt(os.path.join(HERE, "test_data", "ebm3_cams-csm1-0_temperature.txt"))
